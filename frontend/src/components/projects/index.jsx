...
const Projects = ()=> {
    ...
    return (
        <div className="projects sa sa-up">
            <div className="moveButton left" onClick={(e)=>{moveHandler(e)}}><img src={process.env.PUBLIC_URL+'/icon/left.png'}/></div>
            {createCard(cIdxs[0])}
            {createCard(cIdxs[1])}
            <div className="moveButton right"onClick={(e)=>{moveHandler(e)}}><img src={process.env.PUBLIC_URL+'/icon/right.png'}/></div>
        </div>
    )
}
